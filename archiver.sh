...
( for feed in $(ls "$myhour" -1|cut -d_ -f1|sort -u );do  ls -1 "/tmp/urls.$feed/" |while read n;do test -e /tmp/seen.$feed/$n && echo $feed/$n;done;done| xargs -P 1 -n $BATCHSIZE |while read sumlist;do 
  ...
  hictr=0
  ( cat /tmp/urls.$feed/$m|grep -q -e ^http:// -e ^ftp:// -e ^redis:// -e ^rediss:// -e ^https:// -e ^dav:// -e ^davs:// -e ^smb:// -e ^s3:// ) && for elem in $sumlist;do 
  ...
  echo "--" $loctr
  [[ "$loctr" == 0 ]] ||( echo -n "↓" ;curlres=$(curl -s -H "API-KEY: $TSTOKEN" "${TSURL}" -H "Content-Type: application/json" -X POST  --data "${lojsonout}" );(echo "$curlres";echo ) |grep -v ^$ )  & 
  [[ "$loctr" == 0 ]] || sleep 0.2
  ...
  echo "++" $hictr
  [[ "$hictr" == 0 ]] ||  ( echo -n "↑" ;curlres=$(curl -s -H "API-KEY: $TSTOKEN" "${TSURL}" -H "Content-Type: application/json" -X POST  --data "${hijsonout}" );(echo "$curlres";echo ) |grep -v ^$ ) & 
