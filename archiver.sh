...
test -e /tmp/.del_$myhour || touch "/tmp/.del_$myhour"
  ...
  echo restic backup --time "$timestamp" --host "$hostname" "$myhour/"${arch/\.gz/} 
       restic backup --time "$timestamp" --host "$hostname" "$myhour/"${arch/\.gz/} && echo "$myhour/$arch" >> "/tmp/.del_$myhour"
  ...
  echo "$links"| xargs -P 1 -n 22|while read list;do 
    ...
    curl -s -H "API-KEY: $TSTOKEN" "${TSURL}" -H "Content-Type: application/json" -X POST  --data "${jsonout}"|jq .res.status|grep -v '":0'  -c ;echo
  done
...
mkfifo /tmp/rst.io || true 

[[ "$SECREADY" = "true" ]] && {  export RESTIC_REPOSITORY="$SECRESTURL";export AWS_SECRET_ACCESS_KEY="$SECRESTSKY";export AWS_ACCESS_KEY_ID="$SECRESTACK" ; restic copy --from-repo /tmp/restic &> /tmp/rst.log ;  cat /tmp/rst.log 2>&1|grep -v ^$|sed 's/^/'"$myhour"'| SEC:/g' ; restic forget --keep-hourly 2 --prune 2>&1|grep -v ^$|sed 's/^/'"$myhour"'| SEC:/g'  ;  } &
sleep 2
export RESTIC_REPOSITORY="$RESTURL";export AWS_SECRET_ACCESS_KEY="$RESTSKY";export AWS_ACCESS_KEY_ID="$RESTACK"
BACKUP_OK=false
...
export RESTIC_REPOSITORY="$RESTURL";export AWS_SECRET_ACCESS_KEY="$RESTSKY";export AWS_ACCESS_KEY_ID="$RESTACK"
restic forget --keep-hourly 2 --prune 2>&1|grep -v ^$|sed 's/^/'"$myhour"'| PRI:/g'

[[ "$SECREADY" = "true" ]] && {  export RESTIC_REPOSITORY="$SECRESTURL";export AWS_SECRET_ACCESS_KEY="$SECRESTSKY";export AWS_ACCESS_KEY_ID="$SECRESTACK" ; restic forget --keep-hourly 2 --prune 2>&1|grep -v ^$|sed 's/^/'"$myhour"'| SEC:/g'  ; } ; 