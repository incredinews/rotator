...
LABEL org.opencontainers.image.source=https://github.com/incredinews/rotator
...
RUN apt-get update && (apt-get install -y --no-install-recommends curl docker-compose-plugin socat docker.io docker-compose python3-pip python3-venv python3-full  cython3  jq bash git 2>&1|grep -v "Get:") && (apt-get clean all ||true) && which jq && which git && which curl  
